...
GRAVITY = 0.9          # Zwiększone z 0.5 (szybsze spadanie)
JUMP_STRENGTH = -12    # Zwiększone z -8  (mocniejszy skok w górę)
...
class BirdGame:
    def __init__(self, cam_index=0):
        ...
        self.was_closed = False # Do wykrywania momentu zaciśnięcia

        # --- Stan Gry ---
        self.reset_game()
        self.game_active = False # Stan oczekiwania na start

    def reset_game(self):
        # Ptak (prosty kwadrat na początek)
    ...
    def create_pipe(self):
        # Losowa wysokość dolnej krawędzi górnej rury
        # Marginesy (50px) zapewniają, że rura nie jest przyklejona do samej krawędzi
        min_height = 100
        max_height = SCREEN_HEIGHT - 100 - PIPE_GAP
        
        random_pipe_pos = random.randint(min_height, max_height)
        
        # Dolna rura (zaczyna się pod przerwą i idzie do dołu ekranu)
        ...
        # Górna rura (zaczyna się na górze i kończy przed przerwą)
    ...
    def move_pipes(self, pipes):
        ...
        # Usuwamy rury, które wyszły poza ekran
    ...
    def draw_elements(self, pipes, bird_rect, score):
        ...
        # Rury (rysujemy z zaokrąglonymi rogami dla uroczego efektu)
        for pipe in pipes:
            pygame.draw.rect(self.screen, COLOR_PIPE, pipe, border_radius=10)
            # Opcjonalnie: ciemniejszy obrys
    ...
    def run(self):
        running = True
        while running:
            # 1. Obsługa zdarzeń Pygame (klawiatura)
            ...
            # 2. Aktualizacja Trackera (Kamera)
            ...
            # Zapamiętujemy stan na następną klatkę
            ...
            if self.game_active:
                ...
                # Skok (klawiszem spacji LUB zaciśnięciem pięści)
                ...
                # Naliczanie punktów: 1 punkt za każdą miniętą rurę (parę rur)
                # Sprawdzamy, czy środek rury minął środek ptaka
                # (Prostsza wersja: +1 za czas przeżycia, tutaj +0.05 za klatkę dla płynności)
                self.score += 0.05 
            ...
            # Ekran startowy / Game Over
            if not self.game_active:
                overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                overlay.fill((0, 0, 0, 120)) # Półprzezroczyste tło
                ...
                # Jeśli wykryto skok na ekranie Game Over, restartujemy grę
                if jump_triggered:
                     self.reset_game()
                     self.game_active = True

            # Wizualizacja trackera w rogu (pomocnicze)
            if is_closed:
